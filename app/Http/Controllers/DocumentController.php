...
{
    public function index_recieve(Request $request,$idstore,$iduser)
    {
        $year = date('Y');
        $idstore =  Auth::user()->store_id;
        $iduser =  Auth::user()->id;

        $allData = Document::leftjoin('clinic_locate','document.DOC_STORE_ID','=','clinic_locate.LOCATE_ID')
        ->where('document.DOC_STORE_ID','=',$idstore)->get();
    ...
    public function index_recieve_add(Request $request,$idstore,$iduser)
    {
        $year = date('Y');
        // $idstore =  Auth::user()->store_id;
        // $iduser =  Auth::user()->id;
        ...
        return view('document.index_recieve_add',[
            'data_hos'=>$data_hos,'pms'=>$pm,'permiss_list'=>$permiss_list,   'permiss'=>$permiss,'permiss_u'=>$permiss_u,
            'product_a' => $product_a,'locate_a' => $locate_a,'category_a' => $category_a,'unit_a' => $unit_a,'supplier_a' => $supplier_a,
            'sym_a' => $sym_a,'recieve_a' => $recieve_a,'pay_a' => $pay_a,'per_a' => $per_a,'user_a' => $user_a,'order_a'=>$order_a,
            'allData' => $allData,
    ...
    public function index_send(Request $request)
    {
        $year = date('Y');
        $idstore =  Auth::user()->store_id;
        $iduser =  Auth::user()->id;
        ...
        return view('document.index_send',[
            'data_hos'=>$data_hos,'pms'=>$pm,'permiss_list'=>$permiss_list,   'permiss'=>$permiss,'permiss_u'=>$permiss_u,
            'product_a' => $product_a,'locate_a' => $locate_a,'category_a' => $category_a,'unit_a' => $unit_a,'supplier_a' => $supplier_a,
            'sym_a' => $sym_a,'recieve_a' => $recieve_a,'pay_a' => $pay_a,'per_a' => $per_a,'user_a' => $user_a,'order_a'=>$order_a,
            'allData' => $allData,
